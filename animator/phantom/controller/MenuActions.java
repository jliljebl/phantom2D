...
public class MenuActions
{
	...
	public static void setRenderSettings()
	{
		...
		MComboBox size = new MComboBox( "Movie size", 75, sizeOtps );

		MInputArea qs = new MInputArea( "Quality and size" );
		qs.add( quality );
		qs.add( size );

		//--- Mevie encoding
		boolean enab = CodecController.movieOn;
		String encProg = CodecController.encProg;

		final MCheckBox movieOn = new MCheckBox( "Create movie file", enab );
		if( CodecController.encProg == null )
			movieOn.setEnabled( false );

		final MTextField mName = new MTextField( "Movie file name", 75, CodecController.movieFileName );

		String[] codecProgs = { "not available" };
		if( encProg != null )
			codecProgs = CodecController.getCodecProgs();
		final MComboBox codecProgram = new MComboBox( "Codec programs", 20, codecProgs );
		if( encProg != null )
			codecProgram.setSelectedIndex( CodecController.encodeProgs.lastIndexOf( encProg ));
		else
			codecProgram.setSelectedIndex( 0 );

		String[] tFormats = {"not available"};
		if( encProg != null )
			tFormats = CodecController.getFormats();
		final String[] formats = tFormats;
		final MComboBox formatSelect = new MComboBox( "Video formats", 75, formats );
		if( encProg != null )
			formatSelect.setSelectedIndex( getIndex( formats, CodecController.cFormat ));
		else
			formatSelect.setSelectedIndex( 0 );

		String[] vCodecs = {"not available"};
		if( encProg != null )
			vCodecs = CodecController.getVCodecs( CodecController.cFormat );
		final MComboBox vCodec = new MComboBox( "Video codec", 20, vCodecs );
		vCodec.setSelectedIndex( getIndex( vCodecs, CodecController.cVCodec ));

		final MTextField videoBit = new MTextField( "Video bitrate kbps", 75, new Integer( CodecController.cVideoBitrate ) );

		/*
		String[] aCodecs = CodecController.getACodecs( CodecController.cFormat );
		final MComboBox audioCodec = new MComboBox( "Audio codec", 75, aCodecs );
		audioCodec.setSelectedIndex( getIndex( aCodecs, CodecController.cACodec ));

		final MTextField audioBit = new MTextField( "Audio bitrate kpbs", 75,  new Integer( CodecController.cAudioBitrate ) );


		MInputArea movieE = new MInputArea( "Movie encoding" );
		movieE.add( movieOn );
		movieE.add( mName );
		movieE.add( codecProgram );
		movieE.add( formatSelect );
		movieE.add( vCodec );
		movieE.add( videoBit );
		//movieE.add( audioCodec );
		//movieE.add( audioBit );

		//--- panel
		MInputPanel rsPanel = new MInputPanel( "Quality and Encoding" );
		rsPanel.add( qs );
		rsPanel.add( movieE );

		vCodec.setEnabled( enab );
		mName.setEnabled( enab );
		codecProgram.setEnabled( enab );
		formatSelect.setEnabled( enab );
		videoBit.setEnabled( enab );
		//audioCodec.setEnabled( enab );
		//audioBit.setEnabled( enab );

		codecProgram.addActionListener
		(
			new MActionListener() 
			{
				public void actionPerformed(ActionEvent e)
				{
					CodecController.setCurrentEncoder( (String) codecProgram.getSelectedItem() );
					String[] ff = CodecController.getFormats();
					if( ff == null ) System.out.println("adadaasdasd");
					formatSelect.setOptions( CodecController.getFormats() );
				}
			}
		);

		formatSelect.addActionListener
		(
			new MActionListener() 
			{
				public void actionPerformed(ActionEvent e)
				{
					int newformat = formatSelect.getSelectedIndex();
					if( newformat == -1 ) return;
					CodecController.cFormat = (String) formatSelect.getSelectedItem();
					vCodec.setOptions( CodecController.getVCodecs( CodecController.cFormat ) );
					//audioCodec.setOptions( CodecController.getACodecs( CodecController.cFormat ) );
				}
			}
		);

		movieOn.addActionListener
		(
			new MActionListener() 
			{
				public void actionPerformed(ActionEvent e)
				{
					...
					boolean enabled = movieOn.getValue();

					vCodec.setEnabled( enabled );
					mName.setEnabled( enabled );
					codecProgram.setEnabled( enabled );
					formatSelect.setEnabled( enabled );
					videoBit.setEnabled( enabled );
					//audioCodec.setEnabled( enabled );
					//audioBit.setEnabled( enabled );
				}
			}
		);

		int retVal = DialogUtils.showMultiInput( rsPanel );
		if( retVal != DialogUtils.OK_OPTION ) return;
		...
		//--- Set encode values.
		CodecController.movieOn = movieOn.getValue();
		CodecController.encProg = (String) codecProgram.getValue();
		CodecController.movieFileName = mName.getStringValue();
		CodecController.cFormat = (String) formatSelect.getValue();
		CodecController.cVCodec = (String) vCodec.getValue();
		//CodecController.cACodec = (String) audioCodec.getValue();
		CodecController.cVideoBitrate = videoBit.getIntValue();
		//CodecController.cAudioBitrate = audioBit.getIntValue();
		*/
	}
	...
	{		
		...
		/*
		MTextField cacheShare = new MTextField( "Cache share %", LEFT, RIGHT,  new Integer( (int) (( 1.0f - MemoryManager.PREVIEW_SHARE ) * 100.0f ) ));
		MTextField viewshare = new MTextField( "ViewEditor share of cache %", LEFT, RIGHT, new Integer( ((int) (MemoryManager.VIEW_EDITOR_SHARE * 100.0f )) ));
		cacheShare.setTextFieldSize( 50 );
		viewshare.setTextFieldSize( 50 );

		MInputArea cArea = new MInputArea( "Memory Management");
